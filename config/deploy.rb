...
namespace :server_setup do
  ...
  task :gem_install, :roles => :app do
    ...
    run "gem install passenger --version 4.0.20"
  end
...
namespace :worker do
  desc "Start workers"
  task :start do
    run "cd #{current_path} && script/delayed_job start", :env => {'RAILS_ENV' => stage}
  end

  desc "Stop workers"
  task :stop do
    run "cd #{current_path} && script/delayed_job stop", :env => {'RAILS_ENV' => stage}
  end
end
